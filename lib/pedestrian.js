...
var match = function(path, patterns, files) {
  return _.chain(patterns).reduce(function(memo, pattern) {
    return memo.concat(files.filter(minimatch.filter(path + '/' + pattern)));
  }, []).uniq().value();
};
  ...
  walk: $('s+a|[s]f', function(path, patterns, callback) {
    if (callback) {
      walk(path, function(err, files) {
        if (err) callback(err);
        if (patterns.length) callback(null, match(path, patterns, files));
        else callback(null, files);
      });
    } else {
      return patterns.length ? match(path, patterns, walk(path)) : walk(path);
    }
  })
};
